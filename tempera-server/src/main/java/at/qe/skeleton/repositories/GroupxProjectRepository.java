...
public interface GroupxProjectRepository extends AbstractRepository<GroupxProject, GroupxProjectId> {
    ...
    @Query("SELECT new at.qe.skeleton.model.dtos.GroupxProjectStateTimeDbDto(gxp.project.id, gxp.group.id, gxp.isActive, er.state, ir.start, ir.end) From GroupxProject gxp JOIN gxp.internalRecords ir JOIN ir.externalRecord er Join gxp.project pr where pr.manager.username = :manager")
    public List<GroupxProjectStateTimeDbDto> getAllgxpStateTimeDtosByManager(String manager);

    @Query("SELECT new at.qe.skeleton.model.dtos.GroupxProjectStateTimeDbDto(gxp.project.id, gxp.group.id, gxp.isActive, er.state, ir.start, ir.end) From GroupxProject gxp JOIN gxp.internalRecords ir JOIN ir.externalRecord er JOIN gxp.group g where g.groupLead.username = :groupLead")
